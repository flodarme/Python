...
def score(nice,mean,name):
        ...
        if nice > 2:    #if condition if valid, call win function passing in the variables so it can use them
            win(nice,mean,name)
        if mean > 2:    #if condition if valid, call lose function passing in the variables so it can use them
            lose(nice,mean,name)
...
def reset(nice,mean,name):
    nice = 0
    mean = 0
    # Notice, the name variable is not reset as the same user has elected to play again
